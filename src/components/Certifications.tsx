...
const Certifications = () => {
  ...
  return (
    <section id="certifications" className="py-20 bg-gradient-to-br from-lemon-yellow/10 via-white to-bubble-pink/10 polka-dots-subtle">
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {certifications.map((cert, index) => (
            <div 
              key={cert.title}
              className={`group card-hover bg-white rounded-3xl overflow-hidden shadow-xl transform hover:scale-105 transition-all duration-500 delay-${index * 100} relative`}
            >
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                  <div className="bg-white/90 backdrop-blur-sm rounded-full p-3">
                    <Award className="w-8 h-8 text-bubble-pink" />
                  </div>
                </div>
              </div>
              ...
              <div className="group-hover:opacity-0 transition-opacity duration-300 p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="bg-gradient-to-r from-bubble-pink to-aqua-blue rounded-full p-3">
                    <Award className="w-6 h-6 text-white" />
                  </div>
                  <span className="font-comic text-sm text-gray-500">{cert.date}</span>
                </div>

                <h3 className="font-bubblegum text-lg font-bold text-gray-800 mb-2">
                  {cert.title}
                </h3>
                <p className="font-comic text-bubble-pink font-bold mb-4">
                  {cert.issuer}
                </p>
                ...
                <Button 
                  size="sm"
                  className="bounce-button bg-gradient-to-r from-bubble-pink to-aqua-blue hover:from-hot-pink hover:to-neon-green text-white font-comic rounded-full border-2 border-black text-xs w-full"
                >
